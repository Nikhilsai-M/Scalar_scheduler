...
export default function NewEventTypeForm({ schedules }: { schedules: ScheduleOption[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFeedback(error instanceof Error ? error.message : "Could not create this event type.");
      setSaving(false);
    }
  };
  ...
  const updateQuestion = (index: number, field: keyof InviteeQuestionDraft, value: string | boolean) => {
  ...
  return (
    <div className={styles.container}>
      <header className={styles.header}>
        <button className={styles.backButton} onClick={() => router.back()}>Cancel</button>
        <div>
          <h1 className={styles.title}>Add One-on-One Event Type</h1>
          <p className={styles.subtitle}>Create a polished booking page with a clean title, slug, duration, buffers, and custom questions.</p>
        </div>
      </header>
          ...
          <div className={styles.formGroup}>
            <div className={styles.sectionHeader}>
              <div>
                ...
                <p className={styles.sectionHint}>Ask for extra context before the meeting is confirmed.</p>
              </div>
              <button
                type="button"
